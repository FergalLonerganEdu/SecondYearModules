...
int main()
{
    ...
    char repeat [1]; // used as condition for while loop to repeat process
    char repeat_yn; // character read from keyboard and used to repeat process if desired
    ...
    printf("\n------------------------------------\n");
    repeat[0] ="y"; // setting boundary condition for while to true

    //-------------------------------------------------------------------------------------------------------------------

    // Main loop to send messages and receive responses
    // This example assumes client will send first.
    while (stop == 0 && repeat[0]=="y")
    {
        printf("Do you wish to download or upload?"); //instructions for user
                ...
                if(header[0]=='y') // indicates the desired file has been found on server.
                {
                    ...
                    while(bytes_rec<nByte && stop==0)                        //while the amount of bytes received/written is still less than the total number of data bytes
                    {
                        ...
                        else     //data is received from server
                        {
                            ...
                            percentage = (bytes_rec/(double)nByte)*100;
        ...
        if(downup[0]=='u'||downup[0]=='U')          // if the user inputs u or upload, enters this function
        {
            ...
            else                                    // if the file is found
            {
                    ...
                    else if(ACK[0]=='y') //acknowledgement 'y' is received.
                    {
                        ...
                        while(bytes_send<nByte)   //while the number of bytes sent is less than the total number of bytes in the file
                        {
                            ...
                            else
                            {
                                ...
                                percentage = (bytes_send/(double)nByte)*100;
                                if(percentage>threshold)
                                {
                                    printf("===%d%%",threshold);
                                    threshold+=10;
                ...
                printf("If yes type a y if no typ an n\n");
                scanf("%c\n",&repeat_yn);
                repeat[0] = repeat_yn;
                if (repeat[0]=="y")
                {
                    printf("Repeating Process\n");
                    send(clientSocket,repeat,1,0);
                }
                 if (repeat[0]=="n")
                {
                    printf("\n");
