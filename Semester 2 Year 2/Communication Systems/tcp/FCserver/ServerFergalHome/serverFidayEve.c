...
int main()
{
    ...
    do
    {
        ...
        else // we got some data
        {
            ...
            //seek and tell to find size of file, on BB.
            ...
            nByte = atoi(number);  // extracts our number from the char string into an int // UPLOAD
            nByte = 107;
            ...
            if(mode=='D')
            {
                ...
                //download functions
                send_File(cSocket, fpi, file_found , nByte); //should return a success file, where client can go around again.
                stop = 1;
            }
            /*
                        if(mode='U')
...
int receive_File(SOCKET cSocket, FILE *fpo, char request[], int nByte)
{
    ...
    int retVal=0,ret=0,i=0;  // return code from functions
    ...
    printf("Receive: Ready to receive upload %d\n|%s|\n", ret, data);
...
int send_File(SOCKET cSocket, FILE *fpi, char file_found, int nByte)
{
    ...
    ret = fseek(fpi, 0,SEEK_END);  // set position to end of file
    nByte = ftell(fpi); // set nbytes to the number of data bytes we wish to send
    ...
    printf("File Sent\n");
    return(0) ;  // indicate success or failure
